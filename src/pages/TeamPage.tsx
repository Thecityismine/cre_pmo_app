...
function ContactCard({ contact, onDelete }: { contact: Contact; onDelete: (id: string) => void }) {
  ...
  return (
    <div className="bg-slate-800 border border-slate-700 rounded-xl p-4 hover:border-slate-600 transition-colors group">
...
function AddContactModal({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative z-10 w-full sm:max-w-md bg-slate-900 border border-slate-700 rounded-t-2xl sm:rounded-2xl">
        ...
        <form onSubmit={save} className="p-5 space-y-3">
          ...
          <div className="flex gap-2 pt-1">
            <button type="button" onClick={onClose} className="flex-1 border border-slate-600 text-slate-300 text-sm py-2.5 rounded-lg hover:bg-slate-800">Cancel</button>
          ...
          {roleOptions.map(r => (
            <button
              ...
              className={clsx(
                ...
                roleFilter === r ? 'bg-blue-600 text-white' : 'bg-slate-800 border border-slate-700 text-slate-400 hover:text-slate-200'
              )}
            >
...
function inp() {
  return clsx('w-full bg-slate-800 text-slate-100 text-sm rounded-lg px-3 py-2 border border-slate-700 focus:outline-none focus:border-blue-500 placeholder-slate-500')
}
