...
function UploadModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div>
          ...
          <select
            ...
            className="w-full bg-slate-900 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-blue-500"
          >
        ...
        <div>
          ...
          <input
            ...
            className="w-full bg-slate-900 border border-slate-800 text-slate-200 text-sm rounded-lg px-3 py-2.5 focus:outline-none focus:border-blue-500 placeholder-slate-600"
          />
        </div>
        ...
        <div
          ...
          className={clsx(
            ...
            dragging
              ...
              : 'border-slate-800 hover:border-slate-500 text-slate-400 hover:text-slate-400'
          )}
        >
          <Upload size={20} />
      ...
      <div className="relative max-w-5xl w-full max-h-[90vh] flex flex-col bg-slate-900 rounded-2xl shadow-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex-1 overflow-auto flex items-center justify-center p-4 min-h-0">
          {isImage(current.type) ? (
            ...
            <iframe src={current.url} className="w-full rounded-lg border border-slate-800" style={{ height: 'min(70vh, 600px)' }} title={label} />
          ) : (
...
function EditModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex gap-2 pt-1">
            ...
            <button type="button" onClick={onClose}
              className="border border-slate-800 text-slate-400 text-sm px-4 py-2 rounded-lg hover:bg-slate-900 transition-colors">
              Cancel
            </button>
          </div>
        </form>
...
function DocCard({ doc: d, onOpen, onDelete, onEdit, deleting }: {
  ...
  return (
    <div className="group relative bg-slate-900 border border-slate-800 hover:border-slate-500 rounded-xl overflow-hidden transition-all hover:shadow-lg hover:shadow-black/30 flex flex-col">
      {/* Thumbnail */}
          ...
          {categories.length > 2 && categories.map(c => (
            <button key={c} onClick={() => setCatFilter(c)}
              className={clsx(
                ...
                catFilter === c ? 'bg-blue-600 text-white' : 'bg-slate-900 text-slate-400 hover:text-slate-200 border border-slate-800'
              )}>
